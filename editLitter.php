<html>
	...
	<body>
        ...
        if ($_SERVER['REQUEST_METHOD'] != 'POST') {
            ...
            $stmt = $conn->prepare('select SN,quantity,price,brand,animal from Litter where SN = :SN;');
            ...
            echo "<form method='post' action='editLitter.php'>";
            echo "<table>";
            echo "<tbody>";
            ...
            $_SESSION["editLitter_SN"] = $SN; 
            
        } else { // after submitting form
            
            try {
                
                // update Litter with edits
                $stmt = $conn->prepare("update Litter set brand = :brand, price = :price, quantity = :quantity, animal = :animal where SN = :SN;");
                
                $stmt->bindValue(':brand', $_POST['brand']);
                $stmt->bindValue(':price', $_POST['price']);
                $stmt->bindValue(':quantity', $_POST['quantity']);
                $stmt->bindValue(':animal', $_POST['animal']);
                $stmt->bindValue(':SN', $_SESSION["editLitter_SN"]);
                
                $stmt->execute();
                
                echo "Successfully updated Litter.";
