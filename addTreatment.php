<html>
	...
	<body>
        ...
        if ($_SERVER['REQUEST_METHOD'] != 'POST') {
            ...
            // make fill-in form
            echo "<form method='post' action='addTreatment.php'>";
            echo "<table>";
            echo "<tbody>";
            ...
            echo "<tr><td>Treatment type</td><td><input name='type' type='text' size='25' required></td></tr>";
            ...
            try {
                ...
                // insert into table
                $stmt = $conn->prepare("insert into Treatment(type_of) values (:type);");
                $stmt->bindValue(":type", trim($_POST['type']));
                $stmt->execute();
                
                $treatment = $conn->lastInsertId();
                
                $stmt = $conn->prepare("insert into Performs values(:groomer, :treatment, :animal, now());");
                ...
                echo "Successfully added new treatment record.";
