<html>
	...
	<body>
        ...
        if (!isset($_GET['SN']) && $_SERVER['REQUEST_METHOD'] != 'POST') {
            ...
            echo "Select an Medication:  ";
            echo "<select name='SN' onchange='this.form.submit();'>";
            echo "<option disabled selected value> -- select an Medication -- </option>";
        ...
        if ($_SERVER['REQUEST_METHOD'] != 'POST') {
            ...
            echo "<form method='post' action='editMedication.php'>";
            echo "<table>";
            echo "<tbody>";
            ...
            $_SESSION["editMedication_SN"] = $SN; 
            
        } else { // after submitting form
            
            try {
                
                // update Medication with edits
                $stmt = $conn->prepare("update Medication set type_of = :type_of, price = :price, quantity = :quantity, animal = :animal where SN = :SN;");
                
                $stmt->bindValue(':type_of', $_POST['type_of']);
                $stmt->bindValue(':price', $_POST['price']);
                $stmt->bindValue(':quantity', $_POST['quantity']);
                $stmt->bindValue(':animal', $_POST['animal']);
                $stmt->bindValue(':SN', $_SESSION["editMedication_SN"]);
                
                $stmt->execute();
                
                echo "Successfully updated Medication.";
                
            } catch (PDOException $e) {
                echo "Error: " . $e->getMessage();
            }
            
            unset ($_SESSION["editMedication_SN"]);
