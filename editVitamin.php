<html>
	...
	<body>
        ...
        if (!isset($_GET['SN']) && $_SERVER['REQUEST_METHOD'] != 'POST') {
            ...
            $stmt = $conn->prepare('select SN,type_of,animal from Vitamin order by SN;');
            $stmt->execute();
            
            // select an Vitamin to get to current related info
            echo "<form method='get'>";
            echo "Select an Vitamin:  ";
            echo "<select name='SN' onchange='this.form.submit();'>";
            echo "<option disabled selected value> -- select an Vitamin -- </option>";
        ...
        if ($_SERVER['REQUEST_METHOD'] != 'POST') {
            ...
            $stmt = $conn->prepare('select SN, type_of,price,quantity,animal from Vitamin where SN = :SN;');
            ...
            echo "<form method='post' action='editVitamin.php'>";
            echo "<table>";
            echo "<tbody>";
            ...
            $_SESSION["editVitamin_SN"] = $SN; 
            
        } else { // after submitting form
            
            try {
                
                // update Vitamin with edits
                $stmt = $conn->prepare("update Vitamin set type_of = :type_of, price = :price, quantity = :quantity, animal = :animal where SN = :SN;");
                
                $stmt->bindValue(':type_of', $_POST['type_of']);
                $stmt->bindValue(':price', $_POST['price']);
                $stmt->bindValue(':quantity', $_POST['quantity']);
                $stmt->bindValue(':animal', $_POST['animal']);
                $stmt->bindValue(':SN', $_SESSION["editVitamin_SN"]);
                
                $stmt->execute();
                
                echo "Successfully updated Vitamin.";
                
            } catch (PDOException $e) {
                echo "Error: " . $e->getMessage();
            }
            
            unset ($_SESSION["editVitamin_SN"]);
